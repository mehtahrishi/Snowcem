...
self.addEventListener("message", async (event) => {
  ...
  try {
    ...
    for (let i = 0; i < output.length; i++) {
      ...
      serializableSegments.push({
        ...
        mask: {
          width: maskW,
          height: maskH,
          data: Array.from(binData), // Convert Uint8Array to plain array for clean worker postMessage
        },
      });
    }
    ...
    if (wallSegments.length === 0) {
      // If ADE20K didn't tag "wall" explicitly, check background/room structural segments
