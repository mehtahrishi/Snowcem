...
export default function VideoModal({ isOpen, onClose, videoUrl, title }: VideoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 sm:p-6 bg-slate-950/85 backdrop-blur-md animate-fadeIn overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-slate-900 rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl border border-white/10 p-0.5 bg-gradient-to-r from-[#5B6BB5] to-[#DF3F6F] my-auto max-h-[92vh] flex flex-col">
        <div className="bg-slate-900 rounded-[14px] sm:rounded-[22px] overflow-hidden flex flex-col flex-grow">
          {/* Modal Header */}
          <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-white/10 bg-[#0B0B0E] shrink-0">
            <div className="flex items-center gap-2 min-w-0 pr-2">
              <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-[#DF3F6F] animate-pulse shrink-0"></div>
          ...
          {/* Modal Footer */}
          <div className="px-4 sm:px-6 py-3 sm:py-4 bg-[#0B0B0E] flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-white/10 shrink-0">
            <div className="flex items-center gap-2 text-[11px] sm:text-xs text-slate-300">
              <ShieldAlert className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#DF3F6F] shrink-0" />
              <span>Snowcem Official Media & TV Commercial</span>
            </div>
            <a
              href="#tools"
              onClick={onClose}
              className="w-full sm:w-auto text-center bg-gradient-to-r from-[#5B6BB5] to-[#DF3F6F] hover:opacity-95 px-5 sm:px-6 py-2 sm:py-2.5 rounded-full text-xs font-bold text-white transition-all shadow-md font-heading"
            >
